...
from models.loss import abs_loss, LogisticLossCompute
...
def build_trainer(args, device_id, model, optims, tokenizer):
    ...
    autoenc_loss = abs_loss(model.generator, symbols, tokenizer.vocab, device, train=True,
                            label_smoothing=args.label_smoothing)

    cup_loss = LogisticLossCompute().to(device)

    trainer = Trainer(args, model, optims, tokenizer, autoenc_loss, cup_loss,
...
class Trainer(object):
    ...
    def __init__(self,  args, model,  optims, tokenizer, abs_loss, cup_loss,
        ...
        while step <= train_steps:
            ...
            for i, batch in enumerate(train_iter):
                if self.n_gpu == 0 or (i % self.n_gpu == self.gpu_rank):
                    ...
                    tokens += batch.tgt[:, 1:].ne(self.abs_loss.padding_idx).sum().item()
                    sents += batch.tgt.size(0)
                    summ_sents += sum([min(max(l // (2*self.args.win_size+1), 1), self.args.ranking_max_k) for l in batch.ex_segs])
                    accum += 1
                    if accum == self.grad_accum_count:
                        if self.n_gpu > 1:
                            tokens = sum(distributed.all_gather_list(tokens))
                            sents = sum(distributed.all_gather_list(sents))
                            summ_sents = sum(distributed.all_gather_list(summ_sents))

                        normalization = (tokens, sents, summ_sents)
